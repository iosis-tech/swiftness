...
pub fn eval_oods_polynomial_inner<Layout: StaticLayoutTrait + LayoutTrait>(
	...
	let value = (column_values[Layout::NUM_COLUMNS_FIRST + Layout::NUM_COLUMNS_SECOND] - oods_values[201]).field_div(&felt_nonzero!((point - oods_point_to_deg)));
	let total_sum = total_sum + constraint_coefficients[201] * value;
	
	let value = (column_values[Layout::NUM_COLUMNS_FIRST + Layout::NUM_COLUMNS_SECOND + 1] - oods_values[202]).field_div(&felt_nonzero!((point - oods_point_to_deg)));
