...
def parse(node: AstNode, comment: str = "") -> str:
    match node:
        ...
        case (
            ...
            if isinstance(expr, ExprIdentifier) and expr.name.startswith(
                "dynamic_params."
            ):
        ...
        case (
            CodeElementFuncCall(  # assert_is_power_of_2(x=x, max_pow=log_trace_length);
                func_call=RvalueFuncCall(
                    func_ident=ExprIdentifier(name="assert_is_power_of_2"),
                    arguments=ArgList(args=[lv, rv]),
                )
            )
        ):
            return f"ensure!(is_power_of_2({parse(lv)}), CheckAssertsError::NotPowerOfTwo);"
        ...
        case CodeElementCompoundAssertEq(
            a=ExprDeref(addr=ExprIdentifier(name="range_check_ptr")),
            b=b,
        ):
            return f"ensure!({parse(b)} < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);\n\t"
