...
pub fn check_asserts(
    ...
    let x =
        trace_length.floor_div(&felt_nonzero!(felt!(dynamic_params.range_check_units_row_ratio)));
    ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Index out of range.
    let x = (trace_length
        .floor_div(&felt_nonzero!(felt!(dynamic_params.range_check_units_row_ratio))))
        - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    ...
    let x = trace_length
        .floor_div(&felt_nonzero!(FELT_8 * felt!(dynamic_params.memory_units_row_ratio)));
    ...
    let x =
        trace_length.floor_div(&felt_nonzero!(FELT_16 * felt!(dynamic_params.cpu_component_step)));
    ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Step must not exceed dimension.
    let x = (trace_length
        .floor_div(&felt_nonzero!(FELT_16 * felt!(dynamic_params.cpu_component_step))))
        - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    ...
    let x = (FELT_16 * felt!(dynamic_params.cpu_component_step))
        - (FELT_4 * felt!(dynamic_params.memory_units_row_ratio));
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    ...
    // Uses_pedersen_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_pedersen_builtin) * felt!(dynamic_params.uses_pedersen_builtin))
    ...
    // Uses_ecdsa_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_ecdsa_builtin) * felt!(dynamic_params.uses_ecdsa_builtin))
    ...
    // Uses_bitwise_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_bitwise_builtin) * felt!(dynamic_params.uses_bitwise_builtin))
    ...
    // Uses_ec_op_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_ec_op_builtin) * felt!(dynamic_params.uses_ec_op_builtin))
    ...
    // Uses_keccak_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_keccak_builtin) * felt!(dynamic_params.uses_keccak_builtin))
    ...
    // Uses_poseidon_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_poseidon_builtin) * felt!(dynamic_params.uses_poseidon_builtin))
    ...
    // Uses_add_mod_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_add_mod_builtin) * felt!(dynamic_params.uses_add_mod_builtin))
    ...
    // Uses_mul_mod_builtin should be a boolean.
        ...
        (felt!(dynamic_params.uses_mul_mod_builtin) * felt!(dynamic_params.uses_mul_mod_builtin))
    ...
    // Column index out of range.
    let x = felt!(dynamic_params.num_columns_first)
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.cpu_decode_opcode_range_check_column_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.cpu_update_registers_update_pc_tmp0_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.cpu_update_registers_update_pc_tmp1_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.pedersen_hash0_ec_subset_sum_selector_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.poseidon_poseidon_full_rounds_state0_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.poseidon_poseidon_full_rounds_state1_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.poseidon_poseidon_full_rounds_state2_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    ...
    // Column index out of range.
    let x = felt!(dynamic_params.memory_multi_column_perm_perm_cum_prod0_column)
        - felt!(dynamic_params.num_columns_first);
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    let x = felt!(dynamic_params.num_columns_first) + felt!(dynamic_params.num_columns_second)
    ...
    // Column index out of range.
    let x = felt!(dynamic_params.range_check16_perm_cum_prod0_column)
        - felt!(dynamic_params.num_columns_first);
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    let x = felt!(dynamic_params.num_columns_first) + felt!(dynamic_params.num_columns_second)
    ...
    // Column index out of range.
    let x = felt!(dynamic_params.diluted_check_cumulative_value_column)
        - felt!(dynamic_params.num_columns_first);
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    let x = felt!(dynamic_params.num_columns_first) + felt!(dynamic_params.num_columns_second)
    ...
    // Column index out of range.
    let x = felt!(dynamic_params.diluted_check_permutation_cum_prod0_column)
        - felt!(dynamic_params.num_columns_first);
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    // Column index out of range.
    let x = felt!(dynamic_params.num_columns_first) + felt!(dynamic_params.num_columns_second)
    ...
    // Offset must be smaller than trace length.
    let x =
        trace_length - felt!(dynamic_params.diluted_check_permutation_cum_prod0_offset) - FELT_1;
    ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
    ...
    if felt!(dynamic_params.uses_pedersen_builtin) != FELT_0 {
        ...
        let x = trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.pedersen_builtin_row_ratio)));
        ...
        let x =
            felt!(dynamic_params.pedersen_builtin_row_ratio).floor_div(&felt_nonzero!(FELT_512));
    ...
    if felt!(dynamic_params.uses_range_check_builtin) != FELT_0 {
        ...
        let x = trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.range_check_builtin_row_ratio)));
        ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Step must not exceed dimension.
        let x = (trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.range_check_builtin_row_ratio))))
            - FELT_1;
        ...
        let x =
            felt!(dynamic_params.range_check_builtin_row_ratio).floor_div(&felt_nonzero!(FELT_8));
    ...
    if felt!(dynamic_params.uses_ecdsa_builtin) != FELT_0 {
        ...
        let x =
            trace_length.floor_div(&felt_nonzero!(felt!(dynamic_params.ecdsa_builtin_row_ratio)));
        ...
        let x = (trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.ecdsa_builtin_row_ratio))))
            - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Index should be non negative.
        let x =
    ...
    if felt!(dynamic_params.uses_ec_op_builtin) != FELT_0 {
        ...
        let x =
            trace_length.floor_div(&felt_nonzero!(felt!(dynamic_params.ec_op_builtin_row_ratio)));
        ...
        let x = (trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.ec_op_builtin_row_ratio))))
            - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Index should be non negative.
        let x =
    ...
    if felt!(dynamic_params.uses_keccak_builtin) != FELT_0 {
        ...
        let x = trace_length
            .floor_div(&felt_nonzero!(FELT_16 * felt!(dynamic_params.keccak_row_ratio)));
        ...
        let x = (FELT_16 * trace_length)
            .floor_div(&felt_nonzero!(felt!(dynamic_params.keccak_row_ratio)));
        ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Index out of range.
        let x = ((FELT_16 * trace_length)
            .floor_div(&felt_nonzero!(felt!(dynamic_params.keccak_row_ratio))))
            - FELT_1;
        ...
        let x =
            trace_length - felt!(dynamic_params.keccak_keccak_diluted_column0_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of keccak/keccak/diluted_column0 is too big.
        let x = (felt!(dynamic_params.keccak_row_ratio).floor_div(&felt_nonzero!(FELT_4096)))
        ...
        let x =
            trace_length - felt!(dynamic_params.keccak_keccak_diluted_column1_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of keccak/keccak/diluted_column1 is too big.
        let x = (felt!(dynamic_params.keccak_row_ratio).floor_div(&felt_nonzero!(FELT_4096)))
        ...
        let x =
            trace_length - felt!(dynamic_params.keccak_keccak_diluted_column2_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of keccak/keccak/diluted_column2 is too big.
        let x = (felt!(dynamic_params.keccak_row_ratio).floor_div(&felt_nonzero!(FELT_4096)))
        ...
        let x =
            trace_length - felt!(dynamic_params.keccak_keccak_diluted_column3_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of keccak/keccak/diluted_column3 is too big.
        let x = (felt!(dynamic_params.keccak_row_ratio).floor_div(&felt_nonzero!(FELT_4096)))
    ...
    if felt!(dynamic_params.uses_poseidon_builtin) != FELT_0 {
        ...
        let x = (FELT_2 * trace_length)
            .floor_div(&felt_nonzero!(felt!(dynamic_params.poseidon_row_ratio)));
        ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Index out of range.
        let x = ((FELT_2 * trace_length)
            .floor_div(&felt_nonzero!(felt!(dynamic_params.poseidon_row_ratio))))
            - FELT_1;
        ...
        let x =
            trace_length - felt!(dynamic_params.poseidon_param_0_input_output_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of poseidon/param_0/input_output is too big.
        let x = (felt!(dynamic_params.poseidon_row_ratio).floor_div(&felt_nonzero!(FELT_2)))
        ...
        let x =
            trace_length - felt!(dynamic_params.poseidon_param_1_input_output_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of poseidon/param_1/input_output is too big.
        let x = (felt!(dynamic_params.poseidon_row_ratio).floor_div(&felt_nonzero!(FELT_2)))
        ...
        let x =
            trace_length - felt!(dynamic_params.poseidon_param_2_input_output_suboffset) - FELT_1;
        ensure!(x < FELT_USIZE_MAX, CheckAssertsError::OutOfRange);
        // Offset of poseidon/param_2/input_output is too big.
        let x = (felt!(dynamic_params.poseidon_row_ratio).floor_div(&felt_nonzero!(FELT_2)))
    ...
    if felt!(dynamic_params.uses_range_check96_builtin) != FELT_0 {
        ...
        let x = trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.range_check96_builtin_row_ratio)));
        ensure!(is_power_of_2(x), CheckAssertsError::NotPowerOfTwo); // Step must not exceed dimension.
        let x = (trace_length
            .floor_div(&felt_nonzero!(felt!(dynamic_params.range_check96_builtin_row_ratio))))
            - FELT_1;
